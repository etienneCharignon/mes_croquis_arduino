...
#define NUM_STATES  8  // number of patterns to cycle through
...
void loop()
{
  ...
  switch (pattern)
  {
    ...
    case Paul:
      ...
      if(loopCount % maxLoops == 0) {
        paul_color = getRandomColor();
      }
      paul(loopCount, paul_color);
      break;
  } 
...
void paul(int ledIndex, rgb_color color) 
{

  clearOff();
  int demiTaille = LED_COUNT/2;
  for (int i = max(0, ledIndex-demiTaille); i < min(ledIndex, demiTaille); i++)
  {
    float periode = 10.0;
    colors[i].red = sin(i/periode) * color.red;
    colors[i].green = cos(i/periode) * color.green;
    colors[i].blue = sin((demiTaille-i)/periode) * color.blue;
    colors[i + demiTaille].red = sin(i/periode) * color.red;
    colors[i + demiTaille].green = cos(i/periode) * color.green;
    colors[i + demiTaille].blue = sin((demiTaille-i)/periode) * color.blue;    
  }
}
